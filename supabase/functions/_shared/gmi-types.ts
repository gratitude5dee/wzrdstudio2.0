/**
 * GMI Cloud shared types and payload translation for Deno edge functions.
 * LTX normalization is shared with the browser client through ../../../shared/ltx-fast.ts.
 */

import { buildLtxFastPayload } from './ltx-fast.ts';
...
const SEEDREAM_SIZE_BY_RATIO: Record<string, Record<string, string>> = {
  '1:1': { '2K': '2048x2048', '3K': '3072x3072', '4K': '3200x3200' },
  '16:9': { '2K': '2560x1440', '3K': '3072x1728', '4K': '3840x2160' },
  '9:16': { '2K': '1440x2560', '3K': '1728x3072', '4K': '2160x3840' },
  '4:3': { '2K': '2304x1728', '3K': '3072x2304', '4K': '3520x2640' },
  '3:4': { '2K': '1728x2304', '3K': '2304x3072', '4K': '2640x3520' },
  '3:2': { '2K': '2352x1568', '3K': '3072x2048', '4K': '3840x2560' },
  '2:3': { '2K': '1568x2352', '3K': '2048x3072', '4K': '2560x3840' },
};
...
function resolveSeedreamSize(payload: GmiQueuePayload): string {
  ...
  if (explicit && (explicit.includes('x') || explicit === '2K' || explicit === '3K')) return explicit;
...
function translateKlingPayload(payload: GmiQueuePayload): GmiQueuePayload {
  ...
  const mappedElements = existingElements ?? (elementIds.length > 0 ? elementIds.map((elementId) => ({ element_id: elementId })) : undefined);
  const mappedImages = existingImages ?? (imageUrl ? [{ image_url: imageUrl, type: 'first_frame' }] : undefined);
  const mappedVideos = existingVideos ?? (videoUrl ? [{ video_url: videoUrl, refer_type: asString(payload.refer_type) ?? 'base', keep_original_sound: asString(payload.keep_original_sound) ?? 'yes' }] : undefined);
  const soundInput = asString(payload.sound);
  const sound = hasVideoReference ? 'off' : soundInput === 'on' || soundInput === 'off' ? soundInput : asBoolean(payload.generate_audio) ? 'on' : 'off';
  ...
  return stripUndefined({
    ...
    aspect_ratio: aspectRatio && VALID_KLING_ASPECT_RATIOS.has(aspectRatio) ? aspectRatio : '16:9',
    sound,
...
function translateGeminiFlashImagePayload(payload: GmiQueuePayload): GmiQueuePayload {
  const VALID_GEMINI_SIZES = new Set(['512', '1K', '2K', '4K']);
  const VALID_GEMINI_RATIOS = new Set(['1:1', '3:2', '2:3', '3:4', '4:3', '4:5', '5:4', '9:16', '16:9', '21:9']);

  const images = collectGenericImages(payload);
  const rawSize = asString(payload.image_size) ?? '1K';
  const imageSize = VALID_GEMINI_SIZES.has(rawSize) ? rawSize : '1K';
  const rawRatio = asString(payload.aspect_ratio) ?? asString(payload.aspectRatio) ?? '1:1';
  const aspectRatio = VALID_GEMINI_RATIOS.has(rawRatio) ? rawRatio : '1:1';
  ...
  return stripUndefined({
    ...
    contents: payload.contents, // multi-turn editing (pass-through)
  });
}
...
function translateSeedancePayload(payload: GmiQueuePayload): GmiQueuePayload {
  ...
  const lastFrame = asString(payload.last_frame);
...
function pickGenericPayloadValue(key: string, payload: GmiQueuePayload): unknown {
  ...
  const image = firstDefined(payload.image, asString(payload.image_url), asStringArray(payload.image_urls)[0]);
  const audio = firstDefined(payload.audio, asString(payload.audio_url), asString(payload.voice_sample));
...
export function translateGmiQueuePayload(
  ...
  switch (modelId) {
    ...
    case 'gmi/seedream-5-0-lite':
    case 'seedream-5-0-lite':
    case 'gmi/seedream-4-0-250828':
    case 'seedream-4-0-250828':
    case 'gmi/seedream-3-0-t2i-250415':
...
export function extractGmiMedia(result: unknown, mediaType: 'image' | 'video' | 'unknown' = 'unknown'): GmiExtractedMedia {
  ...
  for (const entry of mediaUrls) {
    ...
    if (url) outputs.push({ id: asString(media.id), url });
  }
  const videoUrl = asString(payload.video_url);
  if (videoUrl && !outputs.some((entry) => entry.url === videoUrl)) outputs.unshift({ url: videoUrl });
  const directUrl = asString(payload.url);
  if (directUrl && !outputs.some((entry) => entry.url === directUrl)) outputs.push({ url: directUrl });
  const thumbnailUrl = asString(payload.thumbnail_image_url);
  if (thumbnailUrl && mediaType === 'image' && outputs.length === 0) outputs.push({ url: thumbnailUrl });
  const primaryUrl = outputs[0]?.url;
  const previewUrl = thumbnailUrl ?? primaryUrl;
  return { primaryUrl, previewUrl, outputs, thumbnailUrl, elementId: extractGmiElementId(payload) };
}
