...
export const TechHighlight = ({ name }: TechHighlightProps) => {
  const getIconAndColor = () => {
    switch (name) {
      case 'Kling AI':
        return { icon: <Sparkles className="w-4 h-4" />, color: 'bg-orange-400/20 text-orange-400' };
      case 'Luma':
        return { icon: <MessageSquare className="w-4 h-4" />, color: 'bg-rose-400/20 text-rose-400' };
      case 'Hailou AI':
        return { icon: <Database className="w-4 h-4" />, color: 'bg-amber-400/20 text-amber-400' };
      case 'Runway':
